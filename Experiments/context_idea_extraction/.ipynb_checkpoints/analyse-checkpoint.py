    ...
    with open(args.in_path, "r", encoding="utf-8") as fin:
        for ln, line in enumerate(fin, 1):
            ...
            try:
                ...
                fflm_val_c = FFLM(abstract, context, tokenizer, model, device)
                fflm_val_i = FFLM(abstract, idea, tokenizer, model, device)
                ...
                ppl_c_vals.append(perplexity(context, tokenizer, model, device))
                ppl_i_vals.append(perplexity(idea, tokenizer, model, device))
