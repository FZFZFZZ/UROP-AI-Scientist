...
IN_PATH  = "Data/MDP/mechanism_exploration/data.jsonl"
OUT_DIR  = "Data/MDP/mechanism_exploration"
OUT_PATH = os.path.join(OUT_DIR, f"extracted.jsonl")
TEMPERATURE = 0
...
def get_valid_response(system_prompt: str, user_prompt: str, model: str, temperature: float) -> dict:
    ...
    # if failed, use gpt-5 instead (1 in 100 chance during testing)
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            raw = get_response("gpt-5", system_prompt, prompt, temperature=temperature)
            obj = strict_json_extract(raw)
            return obj
        except Exception as e:
            last_err = e
    ...
    raise RuntimeError(f"Failed to obtain valid JSON after {MAX_ATTEMPTS} attempts of gpt-4o and gpt-5: {last_err}")
    ...
    with open(IN_PATH, "r", encoding="utf-8") as fin, \
        ...
        for line in fin:
            ...
            try:
                ...
                if written % 10 == 0:
                    print(f"{written}/350", flush=True)
