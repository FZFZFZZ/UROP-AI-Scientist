...
logger = logging.getLogger("iclr-scraper")
...
def fetch_decision_notes_any(session: requests.Session, year: int,
    ...
    for name in invites:
        inv = f"ICLR.cc/{year}/Conference/-/{name}"
        try:
...
def extract_rows(notes: List[Dict[str, Any]], year: int, decision_bucket: str) -> List[Dict[str, Any]]:
    ...
    for note in notes:
        try:
            ...
            # API v2 acceptance signal:
            pdate = note.get("pdate", None)  # non-null means Accepted (publication date)  <-- KEY FIX

            # Secondary heuristics if you want to keep them (may be empty/hidden):
            venue   = note.get("venue", "") or ""   # often blank in raw API payloads
            venueid = note.get("venueid", "") or "" # often blank in raw API payloads
            # content-level fields (may be hidden by readers)
            c_venue   = ((content.get("venue", {}) or {}).get("value", "") or "")
            c_venueid = ((content.get("venueid", {}) or {}).get("value", "") or "")

            # Start with the explicit bucket label (Withdrawn/Reject/Desk...) else ""
            decision = decision_bucket or ""

            # Upgrade to Accept via pdate (most reliable)
            if not decision and pdate:
                decision = "Accept (Published)"

            # (Optional) tiny backstops if venue text is visible
            if not decision and (venueid == f"ICLR.cc/{year}/Conference" or c_venueid == f"ICLR.cc/{year}/Conference"):
...
# ---------------- Orchestration ----------------
def collect_iclr_year(year: int, max_pages: int = 50, include_decisions: bool = True) -> pd.DataFrame:
    session = create_session_with_retries()
    venueid = f"ICLR.cc/{year}/Conference"

    buckets: List[Tuple[str, str]] = [
        ("", ""),  # main pool (will be accepted via pdate)
...
# ---------------- CLI ----------------
def main():
    ap = argparse.ArgumentParser(description="Fetch ICLR notes from OpenReview API v2.")
    ap.add_argument("--year", type=int, default=int(os.getenv("ICLR_YEAR", "2025")),
                    help="ICLR year to fetch (default: 2025)")
    ...
    ap.add_argument("--out-prefix", type=str, default="iclr",
                    help="Output file prefix (default: iclr)")
    args = ap.parse_args()

    year = args.year
    logger.info("Starting ICLR %d collection", year)
    df = collect_iclr_year(year, max_pages=args.max_pages, include_decisions=not args.no_decisions)

    if df.empty:
        logger.warning("No data collected for ICLR %d.", year)
        return 0
